...
void printSuiteResult() {
  if (!g_suites || !g_suite_name) return;
...
int main(int argc, char* argv[]) {
  ...
  for (int i = 1; i < argc; ++i) {
    ...
    else if (std::strcmp(argv[i], "-s") == 0)           g_suites  = true;
    else if (std::strncmp(argv[i], "--build-secs=", 13) == 0) buildSecs = std::atoi(argv[i] + 13);
  }
  ...
  auto testMs = std::chrono::duration_cast<std::chrono::milliseconds>(
